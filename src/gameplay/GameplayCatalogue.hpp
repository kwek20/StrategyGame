...
struct EntityArchetype {
    ...
    std::vector<std::string> canTrain, availableUpgrades;
};
class GameplayCatalogue final {
  public:
    GameplayCatalogue(
        const std::filesystem::path& entities = "assets/gameplay/entities.json",
    ...
    [[nodiscard]] float productionDuration(const std::string& country,
                                           ...
                                           float producerMultiplier = 1.0F) const;
  ...
  private:
    ...
    [[nodiscard]] bool
    matches(const GameplayModifier&, const EntityArchetype&, const EntityArchetype*) const;
};
} // namespace strategy
