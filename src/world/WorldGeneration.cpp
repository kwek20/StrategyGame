...
bool suitable(const Terrain& terrain,float x,float z) {
    ...
    if(std::abs(x)>half||std::abs(z)>half) return false;
    const float normalized=terrain.heightAt(x,z)/Terrain::heightScale;
    if(normalized<0.16F||normalized>0.70F) return false;
...
bool add(World& world,const std::string&type,float x,float z,float rotation) {
    if(overlapsObject(world,{x,z},collisionRadius(type))) return false;
    std::string key="entity."+type;
    Entity& entity=world.createEntity(Text::get(key),type,0);
    entity.kind=EntityKind::resource;entity.resource.emplace();
    entity.transform.position={x,0.0F,z}; entity.transform.rotationDegrees.y=rotation;
    entity.resource.kind=type=="tree"?ResourceKind::wood:type=="stone"?ResourceKind::stone:ResourceKind::gold;
    entity.resource.remaining=type=="tree"?120.0F:type=="stone"?180.0F:140.0F;
    return true;
}
void clusters(World& world,const Terrain& terrain,std::uint32_t seed,
              const std::string&type,int clusterPairs,int perCluster,float spread) {
    std::mt19937 random(seed); std::uniform_real_distribution<float> coordinate(-78.0F,78.0F);
    std::uniform_real_distribution<float> offset(-spread,spread),rotation(0.0F,360.0F);
    int made=0;
    for(int attempt=0;attempt<clusterPairs*80&&made<clusterPairs;++attempt) {
        const float centerX=coordinate(random),centerZ=coordinate(random);
        if(centerX<0.0F) continue;
        if(!suitable(terrain,centerX,centerZ)||!suitable(terrain,-centerX,-centerZ)
            ||!clearOfBases(centerX,centerZ)||!clearOfBases(-centerX,-centerZ)) continue;
        for(int member=0;member<perCluster;++member) {
            const float x=centerX+offset(random),z=centerZ+offset(random);
            if(suitable(terrain,x,z)&&clearOfBases(x,z)) {
                const float angle=rotation(random);
                if(!overlapsObject(world,{x,z},collisionRadius(type))
                    && !overlapsObject(world,{-x,-z},collisionRadius(type))) {
                    add(world,type,x,z,angle); add(world,type,-x,-z,angle+180.0F);
                }
            }
        }
        ++made;
...
void populateResources(World& world,const Terrain& terrain,std::uint32_t terrainSeed) {
    ...
    clusters(world,terrain,terrainSeed^0xA341316CU,"tree",18,3,4.5F);
    clusters(world,terrain,terrainSeed^0xC8013EA4U,"stone",6,2,3.0F);
    clusters(world,terrain,terrainSeed^0xAD90777DU,"gold",5,2,2.5F);
}
}
