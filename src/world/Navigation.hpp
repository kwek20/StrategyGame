...
public:
    static constexpr int side_=96;static constexpr float cell_=2.0F;
    explicit Navigation(const Terrain& terrain);void rebuildTerrain(const Terrain& terrain);
    std::vector<glm::vec3> findPath(const World& world,glm::vec3 start,
                                    glm::vec3 destination,float radius,EntityId ignored);
private:
    std::vector<float> heights_;std::vector<std::uint8_t> terrainPassable_;
