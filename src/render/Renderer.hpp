...
public:
    ...
    void drawWorld(const World& world, const CameraView& camera,const Player* player=nullptr) const;
    void drawResourceHud(const Player& player) const;
    void drawDebugHud(const RtsCamera& camera, std::size_t entityCount) const;
    void drawDetailedDebugHud(const CameraView& camera,const Entity* entity,
    ...
    void drawStartMenu(bool startHovered, bool buildHovered, bool loadHovered,
    ...
    void drawSettings(const GameConfig& config,std::size_t resolution,int hovered,int binding)const;
    void drawBuildHud(PlayerId team, const std::string& entityType,
                      std::size_t entityCount, const std::string& status) const;
    void drawStrategyHud(const World& world, EntityId selected,const Player* player=nullptr) const;
    ...
    [[nodiscard]] std::vector<EntityId> unitsInScreenRectangle(
        const glm::vec2& start,const glm::vec2& end,const World& world,
        const CameraView& camera,PlayerId owner) const;
    void drawUnitSelectionHud(const World& world,
                              const std::vector<EntityId>& selected) const;
    void drawOrderMarkers(const World& world,EntityId selected,
                          const std::vector<EntityId>& selection,
                          const CameraView& camera) const;
    [[nodiscard]] glm::vec3 screenToTerrain(float pixelX, float pixelY,
                                             const CameraView& camera) const;
    [[nodiscard]] EntityId pickEntity(float pixelX,float pixelY,const World& world,
                                      const CameraView& camera,const Player* player=nullptr,
                                      bool currentlyVisibleOnly=false) const;
    void drawEntityOutline(const World& world,EntityId entity,
                           const CameraView& camera,const Player* player=nullptr) const;
    ...
    void drawPauseMenu(bool resumeHovered, bool exitHovered) const;
    void regenerateTerrain(std::uint32_t seed);
    void setFramesPerSecond(float fps) { framesPerSecond_ = fps; }
    [[nodiscard]] float terrainHeightAt(float worldX, float worldZ) const;
    [[nodiscard]] std::size_t loadedModelCount() const { return resources_.modelCount(); }
