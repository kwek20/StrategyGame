...
public:
    ...
    void restorePlayerProgress(PlayerId player,float wood,float stone,float gold,
    ...
    [[nodiscard]] const PlayerRegistry& players() const { return players_; }
    [[nodiscard]] std::uint64_t tick() const { return tick_; }
    [[nodiscard]] std::uint32_t terrainSeed() const { return terrainSeed_; }
...
private:
    ...
    std::unordered_map<PlayerId, std::uint64_t> lastSequence_;
