...
std::uint64_t authoritativeStateChecksum(const World& world,
    ...
    for (const Entity* entity : entities) {
        ...
        hash.value(static_cast<bool>(entity->gatherer)); if(entity->gatherer){hash.value(entity->gatherer.carriedKind);hash.value(entity->gatherer.carriedAmount);hash.value(entity->gatherer.carryCapacity);hash.value(entity->gatherer.gatherPerSecond);}
