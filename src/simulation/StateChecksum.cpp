...
std::uint64_t authoritativeStateChecksum(const World& world,
    ...
    for (const Entity* entity : entities) {
        ...
        hash.value(static_cast<bool>(entity->power)); if(entity->power){hash.value(entity->power.generation);hash.value(entity->power.demand);hash.value(entity->power.supplied);hash.value(entity->power.priority);hash.value(entity->power.state);}
