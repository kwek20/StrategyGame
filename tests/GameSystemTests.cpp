...
int main() {
    ...
    strategy::Entity componentWorker;componentWorker.modelKey="worker";gameplay.initializeEntity(componentWorker);
    strategy::Entity componentHall;componentHall.modelKey="town_center";gameplay.initializeEntity(componentHall);
    valid=valid&&componentWorker.unitControl&&componentWorker.gatherer&&componentWorker.upgrades&&!componentWorker.production&&!componentWorker.buildingUpgrades;
    valid=valid&&!componentHall.unitControl&&!componentHall.gatherer&&componentHall.production&&componentHall.buildingUpgrades&&componentHall.upgrades;
    valid=valid&&std::abs(gameplay.resolve(strategy::GameplayStat::movementSpeed,"germany","unassigned","worker")-7.35F)<0.001F;
    valid=valid&&std::abs(gameplay.resolve(strategy::GameplayStat::gatherRate,"brazil","logistics","worker")-4.84F)<0.001F;
    const float townTraining=gameplay.productionDuration("spain","unassigned","town_center","worker");
    const float outpostTraining=gameplay.productionDuration("spain","unassigned","outpost","worker");
    valid=valid&&gameplay.canTrain("town_center","worker")&&gameplay.canTrain("outpost","worker")&&outpostTraining>townTraining;
    ...
    strategy::Entity& navigationObstacle=navigationWorld.createEntity("Blocker","town_center",0);
    navigationObstacle.transform.position={0.0F,0.0F,0.0F};
    const strategy::Terrain navigationTerrain{123U};strategy::Navigation navigation{navigationTerrain};
    const auto route=navigation.findPath(navigationWorld,{-18.0F,0.0F,0.0F},{18.0F,0.0F,0.0F},strategy::collisionRadius("worker"),999);
    valid=valid&&!route.empty();
    for(const glm::vec3& point:route) {
        const float safe=strategy::collisionRadius("town_center")+strategy::collisionRadius("worker");
    ...
    obstacle.transform.position={beforeCollision.x
        +strategy::collisionRadius("worker")+strategy::collisionRadius("town_center")
        +0.1F,0.0F,beforeCollision.z};
    valid = valid && session.submit({1,4,
        strategy::DirectUnitInputCommand{movingId,{1.0F,0.0F}}});
    session.update(strategy::GameSession::fixedTickSeconds);
    ...
    std::size_t workersBefore=0;strategy::EntityId hallId=0;
    for(strategy::Entity& entity:session.world().entities()) {
        if(entity.modelKey=="worker"&&entity.authority.owner==1)++workersBefore;
        if(entity.modelKey=="town_center"&&entity.authority.owner==1){hallId=entity.id;entity.production.productionSpeedMultiplier=1000.0F;}
    }
    ...
    for(int tick=0;tick<460;++tick)session.update(strategy::GameSession::fixedTickSeconds);
    const strategy::Entity* upgradedHall=session.world().findEntity(hallId);
    std::size_t workersAfter=0;for(const strategy::Entity& entity:session.world().entities())if(entity.modelKey=="worker"&&entity.authority.owner==1)++workersAfter;
    valid = valid && upgradedHall&&upgradedHall->production.level==2
        && upgradedHall->modelKey=="town_center_level_2"&&workersAfter==workersBefore+1;

    strategy::GameSession gatheringSession{321U};strategy::Entity* gatherer=nullptr;
    for(strategy::Entity& entity:gatheringSession.world().entities())if(entity.modelKey=="worker"&&entity.authority.owner==1){gatherer=&entity;break;}
    valid=valid&&gatherer!=nullptr;
    if(gatherer){strategy::Entity& tree=gatheringSession.world().createEntity("Test Tree","tree",0);tree.transform.position=gatherer->transform.position;tree.resource.emplace();tree.resource.kind=strategy::ResourceKind::wood;tree.resource.remaining=2.0F;valid=valid&&gatheringSession.submit({1,1,strategy::GatherResourceCommand{gatherer->id,tree.id}});for(int tick=0;tick<100;++tick)gatheringSession.update(strategy::GameSession::fixedTickSeconds);valid=valid&&gatheringSession.players().find(1)->wood>0.0F;}

    strategy::GameSession intelligenceSession{777U};strategy::Entity* scout=nullptr;strategy::Entity* observed=nullptr;
    for(strategy::Entity& entity:intelligenceSession.world().entities()){if(entity.modelKey=="worker"&&entity.authority.owner==1)scout=&entity;if(entity.modelKey=="worker"&&entity.authority.owner==2)observed=&entity;}
    if(scout&&observed){observed->transform.position=scout->transform.position+glm::vec3{2,0,0};intelligenceSession.update(strategy::GameSession::fixedTickSeconds);const auto& records=intelligenceSession.players().find(1)->intelligence;const auto known=std::find_if(records.begin(),records.end(),[&](const strategy::LastKnownEntity& item){return item.id==observed->id;});valid=valid&&known!=records.end();const glm::vec3 remembered=known!=records.end()?known->position:glm::vec3{};observed->transform.position={65,0,65};intelligenceSession.update(strategy::GameSession::fixedTickSeconds);const auto& after=intelligenceSession.players().find(1)->intelligence;const auto stale=std::find_if(after.begin(),after.end(),[&](const strategy::LastKnownEntity& item){return item.id==observed->id;});valid=valid&&stale!=after.end()&&stale->position==remembered;}else valid=false;
